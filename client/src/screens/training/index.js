...
class TrainingPage extends React.Component<any, TrainingPageProps> {
  ...
  reduceChartData = (prevChartData: number[][], epoch: number, logs: Logs, key: string) => {
    return [
      [
        ...prevChartData[0],
        {
    ...
    this.worker.addEventListener('message', ({ data: { type, ...data } }) => {
      if (type === "generated") {
        ...
        this.setState((prevState:any) => ({
          chartData: {
            loss: this.reduceChartData(prevState.chartData.loss, epoch, logs, 'loss'),
            mse: this.reduceChartData(prevState.chartData.mse, epoch, logs, 'MSE'),
            acc: this.reduceChartData(prevState.chartData.acc, epoch, logs, 'acc'),
          },
          logs,
